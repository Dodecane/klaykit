...
You can opt in to displaying recent transactions within KlayKit's account modal. Note that all transactions are kept in local storage and must be manually registered with KlayKit in order to be displayed.
...
```tsx
import { RainbowKitProvider } from 'klaykit';
...
Transactions can then be registered with KlayKit using the `useAddRecentTransaction` hook.

```tsx
import { useAddRecentTransaction } from 'klaykit';
...
Once a transaction has been registered with KlayKit, its status will be updated upon completion.
