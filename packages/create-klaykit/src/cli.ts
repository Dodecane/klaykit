...
async function run() {
  try {
    ...
    log();
    log(chalk.green(`Welcome to KlayKit!`));
    ...
    log(
      chalk.cyan(`🚀 Creating a new KlayKit app in ${chalk.bold(targetPath)}`)
    );
    ...
    if (process.env.INSTALL_WORKSPACE_KLAYKIT !== 'true') {
      delete pkgJson.dependencies.klaykit;
    }
    ...
    if (process.env.INSTALL_WORKSPACE_KLAYKIT !== 'true') {
      await execa(
        packageManager,
        [packageManager === 'yarn' ? 'add' : 'install', 'klaykit'],
        {
          cwd: targetPath,
          stdio: 'inherit',
        }
    ...
    if (!options.skipGit) {
      ...
      await execa(
        ...
        [
          ...
          '--no-verify',
          '--message',
          'Initial commit from create-klaykit',
        ],
        { cwd: targetPath }
      );
    }

    log(chalk.green(`Done! Thanks for using KlayKit 🙏`));
    log();
    log(
      chalk.cyan(
